...
{
    Task<(bool Success, string Message)> CreateCompanyAsync(CompanyCreateModel model);
}
...
public class CompanyService : ICompanyService
{
    ...
    public CompanyService(RestClient restClient, ILogger<CompanyService> logger)
    {
        _restClient = restClient;
        _logger = logger;
    }

    public async Task<(bool Success, string Message)> CreateCompanyAsync(CompanyCreateModel model)
    {
        try
        {
            var request = new RestRequest("api/company/create", Method.Post);
            request.AddHeader("Accept", "application/json");
            request.AddHeader("Content-Type", "application/json");

            // Demo.Web CompanyCreateEndpointV1에 맞는 요청 객체 구성
            ...
            _logger.LogInformation("회사 생성 API 호출 시작: {Name}", model.Name);

            var response = await _restClient.ExecuteAsync(request);

            if (response.IsSuccessful)
            {
                _logger.LogInformation("회사 생성 성공: {Name}", model.Name);
                return (true, "회사가 성공적으로 생성되었습니다.");
            }
            else
            {
                ...
                _logger.LogError("회사 생성 실패: {Name}, 상태코드: {StatusCode}, 오류: {Error}",
                    model.Name, response.StatusCode, errorMessage);
        ...
        {
            _logger.LogError(ex, "회사 생성 중 예외 발생: {Name}", model.Name);
