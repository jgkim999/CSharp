...
/// <summary>
/// RabbitMQ Multi 방식 메시지 발행 테스트 엔드포인트의 Swagger 문서화를 위한 요약 클래스
/// FastEndpoints의 EndpointSummary를 상속받아 API 문서를 정의합니다
/// </summary>
class TestMqMultiMessageSummary : EndpointSummary
{
    /// <summary>
    /// TestMqMultiMessageSummary의 새 인스턴스를 초기화하고 Swagger 문서 정보를 설정합니다
    /// Multi 방식은 fanout exchange를 통해 모든 컨슈머가 메시지를 수신합니다
    /// </summary>
...
/// <summary>
/// RabbitMQ Multi 방식 메시지 발행을 테스트하는 엔드포인트 클래스
/// FastEndpoints를 사용하여 구현되며, fanout exchange를 통해 모든 컨슈머가 메시지를 수신합니다
...
public class TestMqMultiMessageEndpoint : Endpoint<MqPublishRequest>
{
    ...
    /// <summary>
    /// TestMqMultiMessageEndpoint의 새 인스턴스를 초기화합니다
    ...
    /// POST /api/test/mqmulti 경로로 익명 접근을 허용하며 MqTest 그룹에 속합니다
    /// </summary>
    ...
    /// <summary>
    /// RabbitMQ Multi 방식 메시지 발행 요청을 비동기적으로 처리합니다
    /// OpenTelemetry Activity를 생성하여 분산 추적을 수행하고, fanout exchange를 통해 모든 컨슈머에게 메시지를 발행합니다
    ...
    public override async Task HandleAsync(MqPublishRequest msg, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, nameof(TestMqMultiMessageEndpoint));
        }
