...
/// </summary>
class MqMessagePackMultiEndpointSummary : EndpointSummary
{
    /// <summary>
    /// TestMqMessagePackSummary의 새 인스턴스를 초기화하고 Swagger 문서 정보를 설정합니다
    /// MessagePack 직렬화를 사용하여 타입 정보와 함께 메시지를 전송합니다
    /// </summary>
    public MqMessagePackMultiEndpointSummary()
    {
        Summary = "MQ Multi MessagePack";
...
/// </summary>
public class MqMessagePackMultiEndpoint : Endpoint<MqPublishRequest>
{
    private readonly ILogger<MqMessagePackMultiEndpoint> _logger;
    ...
    public MqMessagePackMultiEndpoint(
        ILogger<MqMessagePackMultiEndpoint> logger,
    ...
    {
        Post("/api/test/MqMessagePackMulti");
        AllowAnonymous();
        //Version(1);
        Group<MqTest>();
        Summary(new MqMessagePackMultiEndpointSummary());
    }
    ...
    public override async Task HandleAsync(MqPublishRequest msg, CancellationToken ct)
    {
        try
        {
            using var activity = _telemetryService.StartActivity(nameof(MqMessagePackMultiEndpoint));

            // MessagePack으로 메시지 발행 (타입 정보 포함)
            await _mqPublishService.PublishMessagePackMultiAsync("consumer-multi-exchange", msg, ct);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, nameof(MqMessagePackMultiEndpoint));
        }
