...
public class SodBackgroundWorker : BackgroundService
{
    ...
    /// <summary>
    /// Initializes a SodBackgroundWorker that processes queued Sod tasks concurrently.
    /// </summary>
    /// <param name="workerCount">Number of concurrent worker loops to run. Defaults to 8.</param>
    ...
    /// <summary>
    /// Starts the configured number of worker tasks that concurrently process items from the background task queue and waits for all workers to complete.
    /// </summary>
    /// <param name="stoppingToken">Cancellation token that signals when workers should stop processing and exit.</param>
    /// <returns>A task that completes when all worker tasks have finished.</returns>
    ...
    /// <summary>
    /// Processes queued background work items on a single worker loop until cancellation is requested.
    /// </summary>
    /// <param name="workerId">Identifier for the worker; used for logging and tracing.</param>
    /// <param name="stoppingToken">Token that signals the worker to stop processing and exit the loop.</param>
    /// <returns>A <see cref="Task"/> that completes when the worker loop exits (typically when cancellation is requested).</returns>
    ...
    /// <summary>
    /// Logs a graceful shutdown message and delegates shutdown to the base BackgroundService.
    /// </summary>
    /// <param name="cancellationToken">Token that can be used to cancel the stop operation.</param>
    /// <returns>A task that completes when the service has finished stopping.</returns>
